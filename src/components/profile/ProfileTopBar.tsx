  ...
  return (
    <div className="flex items-center justify-between gap-4">
      <div>
        ...
        <p className="text-sm text-slate-400">Manage your profile, track activity, and view your account insights.</p>
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="relative w-80">
          ...
          <input
            placeholder="Search cases, projects, keywords..."
        ...
        <div className="relative">
          <button
            onClick={() => setOpen((v) => !v)}
            className="flex items-center gap-2 rounded-xl bg-blue-500 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-600"
          >
            <Zap className="h-4 w-4" /> Quick Actions <ChevronDown className="h-3.5 w-3.5" />
          </button>
          {open && (
            <div className="absolute right-0 z-20 mt-2 w-56 rounded-xl border border-slate-800 bg-slate-900 p-1.5 shadow-xl">
              {["New Project", "Start Research", "Generate Angle", "Invite Team Member"].map((a) => (
                <button key={a} className="w-full rounded-lg px-3 py-2 text-left text-[13px] text-slate-300 hover:bg-slate-800">
                  {a}
                </button>
              ))}
            </div>
