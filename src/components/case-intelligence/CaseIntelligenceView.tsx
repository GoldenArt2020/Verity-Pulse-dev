// src/components/case-intelligence/CaseIntelligenceView.tsx
"use client";
...
import type { CoverageIntelligence } from "@/services/coverageAnalysis";
...
export function CaseIntelligenceView({ caseId, backHref, backLabel }: CaseIntelligenceViewProps) {
  ...
  const [coverage, setCoverage] = useState<CoverageIntelligence | null>(null);
  const [coverageLoading, setCoverageLoading] = useState(false);
  const coverageTriggeredRef = useRef(false);
  ...
  useEffect(() => {
    if (!researched || !caseData || coverageTriggeredRef.current) return;
    coverageTriggeredRef.current = true;
    setCoverageLoading(true);

    fetch("/api/case/coverage-analysis", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div>
      <div className="flex items-center justify-between border-b border-slate-800/60 bg-[rgb(4,9,22)] px-8 py-4">
        <Link href={backHref} className="flex items-center gap-1.5 text-sm font-medium text-slate-400 hover:text-slate-200">
        ...
        <div className="flex flex-col items-center justify-center gap-4 p-16 text-center">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">
              Gathering sources, YouTube coverage, and building an editorial recommendation. This usually takes under a minute.
            </p>
          </div>
      ...
      {!loading && !error && caseData && researched && (
        <div className="grid grid-cols-[1fr_340px] gap-4 p-6">
          <div className="space-y-4">
            <CaseHeader caseData={caseData} onGenerateAngles={handleGenerateAngles} anglesLoading={anglesLoading} />
            ...
            <ResearchStatusBar caseId={caseData.id} />

            <div className="grid grid-cols-2 gap-4">
              <CoverageMap data={coverage?.coverageMap ?? null} loading={coverageLoading} />
              <AngleSaturationTable data={coverage?.angleSaturation ?? null} loading={coverageLoading} />
            </div>

            <EditorialFeedback data={coverage?.editorialFeedback ?? null} loading={coverageLoading} />

            <UntappedAngles data={coverage?.untappedAngles ?? null} loading={coverageLoading} />
          </div>
