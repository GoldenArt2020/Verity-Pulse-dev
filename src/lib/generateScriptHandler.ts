...
import { SCRIPT_WORD_COUNT_OPTIONS, isValidScriptWordCount } from "@/constants/scriptOptions";
import type { ChannelDNA } from "@/services/creatorDNA";

export async function handleGenerateScriptRequest(req: NextRequest) {
  ...
  if (!isValidScriptWordCount(wordCount)) {
    return NextResponse.json(
      { error: `wordCount must be one of: ${SCRIPT_WORD_COUNT_OPTIONS.join(", ")}` },
      { status: 400 }
    );
  }
  ...
  try {
    const result = await generateScriptForAngle(angleId, caseId, channelDNA, wordCount);
